...
{
  [SerializeField]
  private List<TerrainType> regions = new List<TerrainType>();
  ...
  public static Color[] GetColorMap(float[,] noiseMap, TerrainType[] terrainMap, bool gradiate = false)
  {
    ...
    for (int y = 0; y < height; y++)
    {
      ...
      {
        var index = y * width + x;
        colorMap[index] = gradiate ? GetGradiatedColor(terrainMap[index],noiseMap[x,y]) : terrainMap[index].HighColor;
      }
    }
    return colorMap;
  }
